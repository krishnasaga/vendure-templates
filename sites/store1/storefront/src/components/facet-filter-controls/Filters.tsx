					...
					{_facetsWithValues.map((facet: FacetWithValues) => (
						<div key={facet.id} class="border-b border-muted py-6">
							<h3 class="-my-3 flow-root">
								<button
									class="py-3 bg-white w-full flex items-center justify-between text-sm text-muted hover:text-secondary"
									type="button"
								>
						...
						<div class="fixed inset-0 flex z-40">
							<div class="ml-auto relative max-w-xs w-full h-full bg-white shadow-xl py-4 pb-12 flex flex-col overflow-y-auto translate-x-0">
								<div class="px-4 flex items-center justify-between">
									...
									<button
										type="button"
										class="-mr-2 w-10 h-10 bg-white p-2 rounded-md flex items-center justify-center text-muted"
										onClick$={$(async () => {
											onToggleMenu$();
										})}
									>
									...
									{_facetsWithValues.map((facet) => (
										<div key={facet.id} class="border-t border-muted px-4 py-6">
											<h3 class="-mx-2 -my-3 flow-root">
												<button
													class="px-2 py-3 bg-white w-full flex items-center justify-between text-muted hover:text-secondary"
													type="button"
												>
													<span class="font-medium text-primary uppercase">{facet.name}</span>
													<span
